...
export class Alerts extends Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (error != prevProps.error) {
            ...
            if (error.msg.price)
                alert.error(<div className="text-center bg-danger p-1 rounded" style={{ fontSize: 10 }}>Price: {error.msg.price}</div>)
            if (error.msg.quantity)
                alert.error(<div className="text-center bg-danger p-1 rounded" style={{ fontSize: 10 }}>Quantity: {error.msg.quantity}</div>)
            if (error.msg.scheduled_date)
                alert.error(<div className="text-center bg-danger p-1 rounded" style={{ fontSize: 10 }}>Scheduled date: {error.msg.scheduled_date}</div>)
            if (error.msg.status)
                alert.error(<div className="text-center bg-danger p-1 rounded" style={{ fontSize: 10 }}>Status: {error.msg.status}</div>)
